...
let rec g flag env fmap = function (* インライン展開ルーチン本体 (caml2html: inline_g) *)
  | If(c, x, y, e1, e2) ->
     let e1' = g flag env fmap e1 in
     let e2' = g flag env fmap e2 in
     If(c, x, y, e1', e2')
  | While(x, yts, zs, e) ->
     let e = g flag env fmap e in
     While(x, yts, zs, e)
  | Let((x, t), e1, e2) ->
     let e1' = g flag env fmap e1 in
     let e2' = g flag env fmap e2 in
  ...
  | LetRec({ name = (x, t); args = yts; body = e1 }, e2) -> (* 関数定義の場合 (caml2html: inline_letrec) *)
     if is_rec x e1 && is_loop x true e1 then
       ...
       let e1 = g flag env fmap e1 in
       LetRec({ name = (x, t); args = yts; body = e1 }, g flag env fmap e2)
     else
       if not (M.mem x fmap) then
         g flag env fmap e2
       else
         let env = M.add x (yts, e1) env in
         LetRec({ name = (x, t); args = yts; body = g flag env fmap e1}, g flag env fmap e2)
  | App(x, ys) as exp when M.mem x env -> (* 関数適用の場合 (caml2html: inline_app) *)
     let (zs, e) = M.find x env in
     if is_rec x e = false && ((*(M.find x fmap) <= 1 || *)has_while e = false || size e < 300) then
       if (not flag) && (has_while e) then
         exp
       else
       ...
       exp
  | LetTuple(xts, y, e) -> LetTuple(xts, y, g flag env fmap e)
  | e -> e

let f e =
  let e = g false M.empty (h e) e in
  let e = g true M.empty (h e) e in
  e
